    ...
    if not first_name or not gender or not pizza_preference or not pizza_frequency:
        ...
        if not first_char:
            with open('survey.csv', 'w') as csvfile:
                ...
                # write survey data
                writer.writerow({'first_name': first_name, 'gender': gender, 'pizza_preference': pizza_preference, 'pizza_frequency': pizza_frequency})
        else:
...
def get_sheet():
    ...
    return render_template("table.html", table_data=table_data)flas